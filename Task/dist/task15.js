...
task12_1.guestList.unshift({ name: "Hazrat Abu Bakr(R.A)", status: "deceased" });
// Add the guest element at the middel
const middleIndex = Math.floor(task12_1.guestList.length / 2);
task12_1.guestList.splice(middleIndex, 0, {
    name: "Hazrat Umar(R.A)",
    status: "deceased",
});
// Add the guest element at the last
task12_1.guestList.push({ name: "Hazrat Usman(R.A)", status: "deceased" });
function newMessage(guestList) {
